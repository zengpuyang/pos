...
function printInventory(inputs) {var zongji= 0,shuchu='';
    ...
    for(var i=0;i<inputs.length;i++){
        for(var h= 0;h<loadAllItems().length;h++){if(inputs[i]==loadAllItems()[h].barcode){break}}
        zongji+=loadAllItems()[h].price;
        var count=1,c=0;
        for(var k=0;k<i;k++){if(inputs[i]==inputs[k]){c=1}}
        if(c==1){continue}
        for(var j=i+1;j<inputs.length;j++){if(inputs[i]==inputs[j]){count+=1}}
